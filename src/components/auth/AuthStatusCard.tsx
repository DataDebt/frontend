...
function actionButtonStyle(variant?: string): CSSProperties {
...
function StatusAction({ action, variant = "primary" }: StatusActionProps) {
  if (!action) {
    return null;
  }
  ...
  return (
    <>
      ...
      >
          ...
          style={{
            ...
            background:
              status === "error"
                ? "rgba(224,82,82,0.12)"
                : status === "success"
                  ? "rgba(58,173,107,0.12)"
                  : "rgba(45,159,101,0.12)",
          }}
        >
        ...
        {detail ? (
          <p style={{ margin: "12px 0 0", color: C.text, fontSize: 13, lineHeight: 1.6 }}>{detail}</p>
        ) : null}
      </div>
