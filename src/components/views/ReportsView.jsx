...
import {
  ResponsiveContainer, RadarChart, Tooltip, Radar, PolarGrid, PolarAngleAxis, PolarRadiusAxis
  , BarChart, Bar, XAxis, YAxis, CartesianGrid
} from "recharts"
import { radarData } from "@/constants/mockData";
...
export function ReportsView() {
  ...
  const trafficLight = heatData.map(d => ({ ...d, color: d.score < 2 ? C.danger : d.score < 3.5 ? C.warn : C.success, colorBg: d.score < 2 ? "#fde8e8" : d.score < 3.5 ? "#fef3e2" : "#e3f8ed" }));
    ...
    <div style={{ padding: "20px 36px 36px" }}>
      ...
      <div style={{ padding: "20px 0" }}>
        ...
        <Card style={{ marginBottom: 24 }}>
          <label style={{ fontSize: 13, fontWeight: 600, color: C.text, display: "block", marginBottom: 8 }}>Seleccionar evaluación</label>
          <select value={selected || ""} onChange={e => setSelected(Number(e.target.value) || null)}
            ...
            {closed.map(ev => <option key={ev.id} value={ev.id}>{ev.name} ({ev.date})</option>)}
          </select>
          {!closed.length && <p style={{ color: C.textMuted, fontSize: 13, marginTop: 8 }}>No hay evaluaciones cerradas disponibles.</p>}
        </Card>
        ...
        {!selected ? (
          ...
          <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, marginBottom: 20 }}>
              <Card>
                ...
                <ResponsiveContainer width="100%" height={280}>
                  <RadarChart data={radarData}> {/*Datos de la grafica*/}
              ...
              <Card>
                ...
                <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
                  {[["🔴", trafficLight.filter(d => d.score < 2).length], ["🟡", trafficLight.filter(d => d.score >= 2 && d.score < 3.5).length], ["🟢", trafficLight.filter(d => d.score >= 3.5).length]].map(([em, n]) => (
                    <div key={em} style={{ flex: 1, textAlign: "center", background: "#f8faf9", borderRadius: 8, padding: "6px 0", fontSize: 13, fontWeight: 700, color: C.text }}>{em} {n}</div>
                  ))}
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
                  {trafficLight.sort((a, b) => a.score - b.score).map(d => (
                    <div key={d.domain} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 12px", background: d.colorBg, borderRadius: 8, cursor: "pointer" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, marginBottom: 20 }}>
              <Card>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 8 }}>
                  {heatData.map(d => (
                    <div key={d.domain} title={`Score: ${d.score} | Impacto: ${d.impact} | Riesgo: ${d.risk} | Esfuerzo: ${d.effort}`}
                ...
                <ResponsiveContainer width="100%" height={220}>
                  <BarChart data={radarData} layout="vertical">
                    ...
                    <Bar dataKey="score" fill={C.accent} radius={[0, 6, 6, 0]}>
                      {radarData.map((d, i) => <rect key={i} fill={scoreColor(d.score)} />)}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
              </Card>
            ...
            {/* Risk Matrix */}
            <Card>
              <h3 style={{ margin: "0 0 16px", fontSize: 15, fontWeight: 700, color: C.text }}>⚠️ Matriz de Riesgos (Impacto × Riesgo de Materialización)</h3>
              <div style={{ display: "grid", gridTemplateColumns: "60px 1fr 1fr 1fr", gridTemplateRows: "40px 1fr 1fr 1fr", gap: 4 }}>
                ...
                {["", "Bajo", "Medio", "Alto"].map((l, i) => (
                  <div key={i} style={{ display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 700, color: C.textMuted }}>{l}</div>
                ))}
                {["Alto", "Medio", "Bajo"].map((rowLabel, ri) => (
                   <Fragment key={rowLabel}> 
                   <div key={rowLabel} style={{ display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 700, color: C.textMuted, writingMode: "horizontal-tb" }}>{rowLabel}</div>
                    {["Bajo", "Medio", "Alto"].map((col, ci) => {
                      const zone = (ri === 0 && ci === 2) || (ri === 1 && ci === 2) || (ri === 2 && ci === 1) || (ri === 2 && ci === 2) ? "critical" :
                        (ri === 0 && ci === 0) || (ri === 1 && ci === 0) || (ri === 0 && ci === 1) ? "low" : "medium";
                      const zoneBg = zone === "critical" ? "#fde8e8" : zone === "medium" ? "#fff8e3" : "#e3f8ed";
                      const items = heatData.filter(d => d.impact === rowLabel && d.risk === col);
                      return (
                        <div key={col} style={{ background: zoneBg, borderRadius: 8, padding: 8, minHeight: 70, border: `1px solid rgba(0,0,0,0.06)` }}>
                          <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
                            {items.map(d => (
                              <div key={d.domain} title={d.domain} style={{ width: d.effort === "Alto" ? 28 : d.effort === "Medio" ? 22 : 16, height: d.effort === "Alto" ? 28 : d.effort === "Medio" ? 22 : 16, borderRadius: "50%", background: scoreColor(d.score), display: "flex", alignItems: "center", justifyContent: "center", fontSize: 8, color: "#fff", fontWeight: 700, cursor: "pointer" }}>
                                {d.domain.charAt(0)}
                              </div>
                            ))}
                          </div>
              ...
              <p style={{ fontSize: 11, color: C.textMuted, marginTop: 8 }}>* El tamaño del círculo representa el Esfuerzo de Reembolso. Hover para ver el nombre del dominio.</p>
            </Card>
          </>
        )}
      </div>
